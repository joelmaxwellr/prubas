...
const CrearCard=(d)=>
{
  ...
  const colors =[
   ...
   d.stats.forEach(element=>{
     ...
     ol.append(`${element.stat.name.toUpperCase()} - ${element.base_stat}`);
     pstast.append(ol)
     ol.setAttribute("style", `width: ${element.base_stat}%; height: 1%;margin-bottom:0.1em;padding: 0.5em;background-color: green;align-content:end`)
     pstast.className = "stats"
  ...
  /*  for (let i = 0; i < d.stats.length; i++) {
      ...
      console.log("hola" + d.stats[i].base_stat)
   } */
...
/* function cadenaEstadisticas (stats) {
